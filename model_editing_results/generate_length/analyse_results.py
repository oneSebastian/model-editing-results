...
def load_results_data(base_path="model_editing_results/generate_length/rating/rating_results"):
    dfs = []
...
def load_data(add_answer_span=False, save_merged=False):
    ...
    if save_merged:
        df.drop(columns=["answer_aliases"]).to_parquet("model_editing_results/generate_length/rating/merged.parquet")
    return df
