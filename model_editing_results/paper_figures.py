...
def create_overall_table():
    ...
    fig = make_subplots(rows=2, cols=2, horizontal_spacing=0.1, vertical_spacing=0.25, subplot_titles=group_order)
    ...
    #for group_name, group_df in grouped:
        ...
        editors = ["context-retriever", "in-context", "memit", "lora"]
            ...
            for model in models:
                ...
                fig.add_trace(
                    go.Scatter(
                        ...
                        showlegend=show_legend and model == "gpt-j"
                    ),
                    row=row + 1, col=col + 1
                )
            if add_no_edit != "skip":
        ...
        fig.write_image(f"model_editing_results/visualisations/overview.svg", width=490, height=500, engine="kaleido")
        fig.write_image(f"model_editing_results/visualisations/overview.png", width=490, height=500, engine="kaleido")
...
def plot_control_individually():
    ...
    def symlog_transform(y, lin_thresh=1e-2):
        return np.sign(y) * np.log10(1 + np.abs(y) / lin_thresh)

    plot_editors = ["context-retriever", "in-context", "memit", "lora"]
    model_paths = {
        "GPT-J": [
            (1, "experiments/gpt_j/batch_size_1/"),
            (16, "experiments/gpt_j/batch_size_16/"),
    ...
    #df = df[df["editor"] != "lora"]

    dfs_average = []
    dfs = []
    grouped = df.groupby(["model", "task", "metric"])
    for group_name, group_df in grouped:
        group_editors = list(group_df["editor"].unique())
        group_editors.remove("no-edit")
        ...
        #cols = ["context-retriever", "in-context", "memit", "lora"]
        pivot[group_editors] = pivot[group_editors] - no_edit
        dfs.append(pivot)
        avg = pivot.copy()
    ...
    fig_combined = make_subplots(rows=2, cols=3, horizontal_spacing=0.1, vertical_spacing=0.09, subplot_titles=["hellaswag", "lambada", "lambada"])
    target_groups = [
        ("hellaswag", "acc_norm"),
    ...
    for group_name, group_df in grouped:
        ...
        # Create a line for each (model, editor) pair
        for editor in plot_editors:
            for model in models:
        #for model in models:
                df_model = df[df["model"] == model]
        #    for editor in editors:
                if group_name[1] == "perplexity":
                    df_model["y_transformed"] = symlog_transform(df_model[editor])
                    fig.add_trace(
                        go.Scatter(
                            x=df_model["batch_size"],
                            y=df_model["y_transformed"],
                            mode="lines+markers",
                            name=f"{model} {rename_editor(editor)}",
                            line=dict(
                                color=color_map[editor],
                ...
                if group_name in target_groups:
                    ...
                    fig_combined.add_trace(
                        ...
                        row=1 if model == "gpt-j" else 2, col=col,
                    )
        ...
        if group_name[1] == "perplexity":
            yticks_original = [-2, -1, 0, 1, 2, 10, 100, 1000000]
            yticks_transformed = symlog_transform(np.array(yticks_original))
            fig.update_layout(
                yaxis=dict(
                    range=[0, 10],
                    title="Perplexity",
                    tickmode="array",
                    tickvals=yticks_transformed,
                    ticktext=[str(v) for v in yticks_original]
                )
            )
            

        #fig.write_image(f"model_editing_results/visualisations/lm_eval/{group_name}.svg", width=500, height=500, engine="kaleido")
        fig.write_image(f"model_editing_results/visualisations/lm_eval/{group_name}.png", width=500, height=500, engine="kaleido")
    ...
    fig_combined.update_layout(
        yaxis3=dict(range=[-1, 7]),
        yaxis6=dict(range=[-1, 7]),
    )
    fig_combined.update_yaxes(title_text="Norm. Accuracy", row=1, col=1)
    fig_combined.update_yaxes(title_text="Accuracy", row=1, col=2)
    fig_combined.update_yaxes(title_text="Perplexity", row=1, col=3)
    fig_combined.update_yaxes(title_text="Norm. Accuracy", row=2, col=1)
    fig_combined.update_yaxes(title_text="Accuracy", row=2, col=2)
    fig_combined.update_yaxes(title_text="Perplexity", row=2, col=3)
    fig_combined.update_xaxes(title_text="Edit Batch Size", row=2, col=1)
    fig_combined.update_xaxes(title_text="Edit Batch Size", row=2, col=2)
    fig_combined.update_xaxes(title_text="Edit Batch Size", row=2, col=3)
    ...
    # Save the figure as an image
    fig_combined.write_image(f"model_editing_results/visualisations/lm_eval/combined.svg", width=550, height=430, engine="kaleido")
    fig_combined.write_image(f"model_editing_results/visualisations/lm_eval/combined.png", width=550, height=430, engine="kaleido")
    ...
    fig.write_image(f"model_editing_results/visualisations/lm_eval/average.png", width=450, height=450, engine="kaleido")
...
def plot_generate_length():
    ...
    fig.write_image("model_editing_results/visualisations/generate_length/subplots.svg", width=360, height=430, engine="kaleido")
    fig.write_image("model_editing_results/visualisations/generate_length/subplots.png", width=360, height=430, engine="kaleido")
